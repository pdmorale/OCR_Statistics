...
def select_length(length, allow_variable, count, lang_dict):
    """
        From a given source take line and cut-down to a specific num of words.
...
def create_strings_from_file(filename, count):
    ...
    with open(filename, 'r', encoding="utf-8_sig") as f:
        ...
        for astr in strings:
            if astr[-1] is ' ':
                strings[strings.index(astr)] = astr[:-1]

    #print(strings)  # check encoding
    return strings
