...
class FakeTextDataGenerator(object):
    ...
    def generate(cls, index, text, font, out_dir, size, extension, skewing_angle, random_skew, blur, random_blur,
        ...
        # Capture randoms within the class method: cls.value
        ...
        # Capture random int
        cls.value1 = rnd.uniform(0, blur)  # rnd.randint(0, blur)

        final_image = background.filter(
            ImageFilter.GaussianBlur(
                radius=(blur if not random_blur else cls.value1)
            )
        )
        ...
        # Save the image
        cls.value3 = image_name
        final_image.convert('RGB').save(os.path.join(out_dir, image_name))

    @classmethod
    def get_list(cls):

        return [cls.value1, cls.value2, cls.value3]
